...
cau 6:
select dchi_thau, ten_thau 
from chuthau p join cgtrinh q on p.ten_thau=q.ten_thau
               join thietke o on q.stt_ctr=o.stt_ctr
where tinh_thanh='can tho' and hoten_kts='le kim dung';

cau 7:
select noi_tn
from ktrucsu l join thietke k on l.hoten_kts=k.hoten_kts
               join cgtrinh q on k.stt_ctr=q.stt_ctr
where ten_ctr='khach san quoc te' and tinh_thanh='can tho'

cau 8:
select *
from congnhan j inner join thamgia h on j.hoten_cn=h.hoten_cn
                inner join cgtrinh q on h.stt_ctr=q.stt_ctr
where ch_mon='han' or ch_mon='dien' and ten_thau='le van son';

cau 9:
select *
from congnhan j join thamgia h on j.hoten_cn=h.hoten_cn
                join cgtrinh q on h.stt_ctr=q.stt_ctr
where ten_ctr='khach san quoc te' and ngay_tgia between 'dec-15-1994' and 'dec-31-1994';
...
select *
from congnhan j join thamgia h on j.hoten_cn=h.hoten_cn              
                join thietke k on h.stt_ctr=k.stt_ctr              
...
cau 13:
select hoten_kts, min(nams_kts) as "kien truc su tre nhat"
from ktrucsu
group by hoten_kts;

cau 14:
select ten_thau, sum(kinh_phi) as "tong kinh phi"
from cgtrinh
group by ten_thau;

cau 15:
select dchi_thau, min(kinh_phi) as "anh chu thau may man"
from chuthau p join cgtrinh q on p.ten_thau=q.ten_thau
group by dchi_thau; 

cau 16:
select sum(thu_lao) as "tong thu lao"
from ktrucsu l join thietke k on l.hoten_kts=k.hoten_kts
where thu_lao>25;

cau 20:
select distinct cg.stt_ctr, cg.ten_ctr, cg.diachi_ctr, count(hoten_cn)
from cgtrinh cg join thamgia t on cg.stt_ctr=t.stt_ctr
group by cg.stt_ctr, cg.ten_ctr, cg.diachi_ctr having count(hoten_cn)=
(select max(count(hoten_cn))
from thamgia
group by stt_ctr);
